...
class NN_jackal():
    def __init__(self, veh_name, veh_data, nn, actions):
        ...
        self.pub_others = rospy.Publisher('~other_vels',Vector3,queue_size=10)
        self.pub_twist = rospy.Publisher('/cmd_vel',Twist,queue_size=10)
        self.pub_pose_marker = rospy.Publisher('~pose_marker',Marker,queue_size=10)
        self.pub_agent_marker = rospy.Publisher('~agent_marker',Marker,queue_size=10)
        self.pub_agent_markers = rospy.Publisher('~agent_markers',MarkerArray,queue_size=10)
        self.pub_path_marker = rospy.Publisher('~path_marker',Marker,queue_size=10)
        self.pub_goal_path_marker = rospy.Publisher('~goal_path_marker',Marker,queue_size=10)
        ...
        if self.use_clusters:
            ...
            self.sub_clusters = rospy.Subscriber('~clusters', Obstacles, self.newcbClusters)
        else:
    ...
    #chuga
    def newcbClusters(self, msg):
        other_agents = []
        xs = []; ys = []; radii = []; labels = []
        num_clusters = len(msg.circles)
        for i in range(num_clusters):
            index = i
            x = msg.circles[i].center.x
            y = msg.circles[i].center.y
            v_x = msg.circles[i].velocity.x
            v_y = msg.circles[i].velocity.y
            radius = max(PED_RADIUS, msg.circles[i].radius)
            xs.append(x); ys.append(y); radii.append(radius); labels.append(index)
            heading_angle = np.arctan2(v_y, v_x)
            pref_speed = np.linalg.norm(np.array([v_x, v_y]))
            goal_x = x + 5.0; goal_y = y + 5.0
...
def run():
    ...
    veh_data = {'goal':np.zeros((2,)),'radius':0.5,'pref_speed':pref_speed,'kw':10.0,'kp':1.0,'name':'link'}
...
def newcbClusters(msg):
    ...
    for i in range(num_clusters):
        index = i
        x = msg.circles[i].center.x; y = msg.circles[i].center.y
        v_x = msg.circles[i].velocity.x; v_y = msg.circles[i].velocity.y
        # radius = PED_RADIUS
        #lower_r = np.linalg.norm(np.array([msg.mean_points[i].x-msg.min_points[i].x, msg.mean_points[i].y-msg.min_points[i].y]))
        #upper_r = np.linalg.norm(np.array([msg.mean_points[i].x-msg.max_points[i].x, msg.mean_points[i].y-msg.max_points[i].y]))
        #inflation_factor = 1.5
        ...
        if pref_speed < 0.2:
            pref_speed = 0; v_x = 0; v_y = 0
        other_agents.append(agent.Agent(x, y, goal_x, goal_y, radius, pref_speed, heading_angle, index))
        #print(index, x, y, goal_x, goal_y, radius, pref_speed, heading_angle)
    #self.visualize_other_agents(xs, ys, radii, labels)
    #self.other_agents_state = other_agents
'''
