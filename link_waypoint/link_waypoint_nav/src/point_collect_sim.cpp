...
{
    pose_x = pose_goal->pose.pose.position.x;
    pose_y = pose_goal->pose.pose.position.y;
}
...
int main(int argc, char** argv)
{
    ...
    // Initialize node and time
		ros::init(argc, argv, "point_collect_sim"); //initiate node called collect_gps_waypoints
		ros::NodeHandle n;
		ros::Time::init();
		ros::Time time_last;
    ...
    //Initiate subscribers
        ros::Subscriber sub_odometry = n.subscribe("/odometry/filtered_map", 100, get_pose_goal);
        ros::Subscriber sub_simple_goal = n.subscribe("/move_base_simple/goal", 100, get_rviz_goal);
		ROS_INFO("Initiated point_collect_sim node");
		...
		while(continue_collection)
		{
			...
			{	
				...
				{//do not write waypoint
					...
					ROS_WARN("New pose_x: %f   Last Latitude: %f \n", pose_x, pose_x_last );
					ROS_WARN("New pose_y: %f   Last Longitude: %f \n", pose_y, pose_y_last );
				}
